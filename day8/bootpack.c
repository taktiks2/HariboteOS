		...
		if (fifo8_status(&keyfifo)  + fifo8_status(&mousefifo) == 0) {
			...
			if (fifo8_status(&keyfifo) != 0) {
				...
				if (mouse_decode(&mdec, i) != 0) {
					...
					boxfill8(binfo->vram, binfo->scrnx, COL8_008484, 0, 0, 79, 15); // Erase cordinates
					putfonts8_asc(binfo->vram, binfo->scrnx, 0, 0, COL8_FFFFFF, s); // Write cordinates
					putblock8_8(binfo->vram, binfo->scrnx, 16, 16, mx, my, mcursor, 16); // Draw mouse cursor
				}
			}
		}
    }
